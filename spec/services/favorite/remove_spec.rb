...
describe ::Favorite::Remove  do
  subject { ::Favorite::Remove.new(comic_id: '82967') }
  let(:current_user) { User.first }

  context 'When add a comic as favorite' do
    context 'When pass a valid param' do
      before do
        User.new(login: 'user1', password: 'pws', favorite: [123,321,567,82966, 82967,9,4,3423423] ).save!
      ...
      it 'should be return a array without a commic param id' do
        subject.call

        expect(current_user.favorite).to be_kind_of Array
        expect(current_user.favorite.include?(82967)).to be false
      end
    end
  end
end
